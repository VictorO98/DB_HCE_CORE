...
DELIMITER //
CREATE PROCEDURE insertTipoExamen (IN ID INT, IN NOMBRE_TIPOEXAMEN VARCHAR(200))
BEGIN
  IF (SELECT EXISTS (SELECT idTipoExamen FROM TipoExamen WHERE idTipoExamen = ID)) THEN
...
DELIMITER //
CREATE PROCEDURE updateTipoExamen (IN ID INT, IN NOMBRE_TIPOEXAMEN VARCHAR(200))
BEGIN
  IF (SELECT EXISTS (SELECT idTipoExamen FROM TipoExamen WHERE idTipoExamen = ID)) THEN
    START TRANSACTION;
        UPDATE TipoExamen 
...
DELIMITER //
CREATE PROCEDURE insertDiagnostico (IN DIAG VARCHAR(200), IN ID_CITA INT)
BEGIN
    START TRANSACTION;
...
DELIMITER //
CREATE PROCEDURE updateDiagnostico (IN ID INT, IN DIAG VARCHAR(200), IN ID_CITA INT)
BEGIN
  IF (SELECT EXISTS (SELECT idDiagnostico FROM Diagnosticos WHERE idDiagnostico = ID)) THEN
    START TRANSACTION;
        UPDATE Diagnosticos 
...
DELIMITER //
CREATE PROCEDURE insertDiagXTrata (IN IDDIAG INT, IN IDTRATA INT)
BEGIN
    START TRANSACTION;
...
CREATE PROCEDURE deleteDiagXTrata (IN IDBORRAR INT)
BEGIN
