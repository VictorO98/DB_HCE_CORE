...
BEGIN
	...
	ELSE
		...
		IF ROW_COUNT() > 0 THEN
            SELECT 'EL PACIENTE HA SIDO CREADO CON EXITO', (SELECT idPaciente FROM Pacientes WHERE DNI = ID) "ID Paciente";
            COMMIT;
		ELSE
...
BEGIN
	IF (SELECT EXISTS (SELECT DNI FROM Acudientes WHERE DNI = ID)) THEN
		SELECT nombreAcudiente "Nombre Acudiente", DNI "Identificación", fechaNacimiento ,
				telefono "Telefono", sexo "Sexo", idAcudiente "Id Acudiente"
		FROM Acudientes
		WHERE DNI = ID;
	ELSE
...
DELIMITER //
CREATE PROCEDURE insertAcudiente (IN ID BIGINT, IN NOMBRE VARCHAR(30), IN FECHA DATE, IN TELEFONO BIGINT, IN SEXO VARCHAR(30))
BEGIN
	IF (SELECT EXISTS (SELECT DNI FROM Acudientes WHERE DNI = ID)) THEN
	...
	ELSE
		...
		IF ROW_COUNT() > 0 THEN
			SELECT 'EL ACUDIENTE HA SIDO CREADO CON EXITO', (SELECT idAcudiente FROM Acudientes WHERE DNI = ID) "Id Acudiente";
            COMMIT;
        ELSE
...
DELIMITER //
CREATE PROCEDURE updateAcudiente (IN ID BIGINT, IN NOMBRE VARCHAR(30), IN FECHA DATE, IN TELEFONO BIGINT, IN SEXO VARCHAR(30))
BEGIN
	IF (SELECT EXISTS (SELECT DNI FROM Acudientes WHERE DNI = ID)) THEN
		START TRANSACTION;
        UPDATE Acudientes 
...
DELIMITER //
CREATE PROCEDURE insertMedico (IN ID BIGINT, IN NOMBRE VARCHAR(30), IN FECHA DATE, IN TELEFONO BIGINT)
BEGIN
	IF (SELECT EXISTS (SELECT DNI FROM Medicos WHERE DNI = ID)) THEN
...
DELIMITER //
CREATE PROCEDURE updateMedico (IN ID BIGINT, IN NOMBRE VARCHAR(25), IN FECHA DATE, IN TELEFONO BIGINT)
BEGIN
	IF (SELECT EXISTS (SELECT DNI FROM Medicos WHERE DNI = ID)) THEN
		START TRANSACTION;
        UPDATE Medicos
